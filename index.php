...
if ($page === 'admin') {
    ...
    if ($tab === 'subscriptions') {
        $subs = db()->query('SELECT s.*, u.email, p.name AS plan_name FROM subscriptions s JOIN users u ON u.id = s.user_id JOIN plans p ON p.id = s.plan_id ORDER BY s.ends_at DESC LIMIT 50')->fetchAll();
